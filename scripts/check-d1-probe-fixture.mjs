...
check(
    JSON.stringify(workerMutationLifecycle.roles_in_order) === JSON.stringify(["sink", "writer_a", "writer_b"]) &&
        JSON.stringify(workerMutationLifecycle.private_shell_steps_in_order) ===
            JSON.stringify([
...
check(
    workerMutationLifecycle.lost_private_shell_response === "manual_cleanup_required_no_retry" &&
        ...
        workerMutationLifecycle.ambiguous_private_shell_reason === "ambiguous_create" &&
...
check(
    JSON.stringify(deployment.access_provisioning_order) ===
        JSON.stringify([
    ...
    "Access provisioning must create the service token before its exact policy and all routes"
);
