...
const beginIntegrationConnection = async (identifier: string, csrfToken: string): Promise<void> => {
    const body = new URLSearchParams({ _csrf: csrfToken, provider_identifier: identifier });
    const response = await fetch("/actions/integration-connections", {
        method: "POST",
    ...
    if (!response.ok || typeof payload.url !== "string") throw new Error("App connection could not be started");
...
const IntegrationChoices = ({
    ...
    const popularApps = useMemo(
        () =>
            catalogApps
                .filter(app => app.featured_rank !== null)
                .toSorted((left, right) => (left.featured_rank ?? 20) - (right.featured_rank ?? 20)),
        [catalogApps]
    );
    const remainingApps = useMemo(() => catalogApps.filter(app => app.featured_rank === null), [catalogApps]);
        ...
        return (
            <button
                ...
                onClick={() => {
                    ...
                    else void beginIntegrationConnection(app.identifier, csrfToken);
                }}
                ...
                <IntegrationIcon
                    integration={
                        integration ?? {
                            ...
                            icon_data_uri: app.icon_data_uri,
        ...
        <>
            ...
            <div className="organization-layout">
                ...
                <aside className="integration-catalog-sidebar" aria-label="Add an integration">
                        ...
                        {availableApps.slice(0, query.trim().length === 0 ? 8 : 20).map(app => (
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    void beginIntegrationConnection(app.identifier, page.view.csrf_token).catch(() => {
                                        setConnectionError("That app connection could not be started. Try again.");
                                    });
                                }}
                                key={app.identifier}
                            >
                    ...
                    {connectionError === null ? null : <p className="form-error">{connectionError}</p>}
                ...
                <div className="setup-stack">
                    ...
                    >
                        ...
                        <IntegrationChoices
                            ...
                            csrfToken={page.view.csrf_token}
