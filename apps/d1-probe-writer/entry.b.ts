...
import { createD1ProbeWriterHttpHandlerV1 } from "./src/http.js";
...
export class D1ProbeWriterBService extends WorkerEntrypoint<Env> {
    recordReceipt(input: unknown): Promise<D1ProbeReceiptResponseV1> {
        return forwardProbeReceiptV1(this.env.PROBE_SINK, "writer_b", input);
    }

    reserveGateway(input: unknown): Promise<D1ProbeGatewayReservationResponseV1> {
        return reserveAndDispatchGatewayProbeV1(this.env.PROBE_DB, this.env.PROBE_SINK, "writer_b", input);
    }

    runGatewayTrial(input: unknown): Promise<D1ProbeGatewayTrialResponseV1> {
        return runGatewayTrialV1(this.env.PROBE_DB, this.env.PROBE_SINK, "writer_b", input);
    }
}
...
export default {
    fetch(request: Request, env: Env, context: ExecutionContext): Promise<Response> {
        return createD1ProbeWriterHttpHandlerV1(
            ...
            { runGatewayTrial: input => runGatewayTrialV1(env.PROBE_DB, env.PROBE_SINK, "writer_b", input) },
