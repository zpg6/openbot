...
OpenBot's first verified slice does one job. An authenticated organization owner starts one short read-only run against one reviewed Metorial connector. The organization may curate more catalog entries, but one Bot revision selects at most four named organization tool policies and four declarative skills. The run uses one provider deployment, one auth config, one OpenRouter model, and one OpenRouter provider. Before the run starts, the user sees the prompt, allowed tools, selected skills, possible data classes, limits, and every possible disclosure destination. Model-selected arguments and returned data do not exist yet and cannot be shown in advance.
...
We will not use OpenCode. It solves a coding-agent problem and brings shell, filesystem, plugins, executable skills, subagents, and another permission system. We do not need the Cloudflare Agents SDK or AI SDK either. The Agents MCP manager persists connection configuration for wake and reconnect, while this design forbids the bearer URL in DO storage. A plain SQLite-backed Durable Object, narrow private gateways, direct vendor clients, and a manual tool loop cover the first release.

Cloudflare Code Mode, Sandbox, Browser Rendering, Workflows, Agents, and model-written code stay out of the first release. The runtime does not resume an uncertain model or tool call.

A Sandbox becomes useful if OpenBot later executes user code, needs a process boundary, or can prove a destination-level network policy that Durable Objects lack. It does not replace control-plane authorization or the capability gateway. Adding it to this read-only loop would add provisioning and cleanup without enforcing a missing product decision.
...
OpenBot adopts that roster-first hierarchy but shows authority before each run. A Grok Bot login or shared computer is available across a user's Bots. An OpenBot connection grants nothing by itself, every run gets a fresh execution object, and the runtime cannot browse or use a computer. The UI must make that difference obvious.

| Grok Bot pattern | OpenBot first-release decision |
|---|---|
| Named Bot roster in the sidebar | Show named Bots with title, color, and derived status. A Bot owns append-only behavior revisions and scoped grants. |
...
| Inline Connect card | Show a server-authored `Connect a source` card only when the Bot lacks a usable grant. The card links to the reviewed connector flow. The model cannot request an arbitrary connection. |
...
| Inline approval card | Show one start-confirmation card with the prompt, tools, possible data classes, destinations, expiry, and limits. It has `Start run` and `Discard`. It is not an action approval or an `Always allow` rule. |
| Result and file cards | Show the plain-text final answer, verified structured source references, resolved model and provider, evidence completeness, and a link to the audit record. File attachments and generated artifacts arrive with the artifact workspace. |
...
The Bot page uses document scrolling, a sticky header, and a composer after the feed. The composer may become sticky only when it cannot cover the last card, focused control, or validation error. The header shows the Bot's fixed-palette color mark, name, active revision, and an `Access` control instead of a computer-view control. `Tasks`, `Access`, and `Profile` use ordinary links below the header. Cosmetic profile fields are name, title, palette color ID, and generated initials. Changing the job, standing instructions, selected organization tools or skills, connector revision, model route, limits, artifact mounts, or data rules creates a new `bot_revision`.

The task feed orders independent runs by OpenBot commit time. A user's prompt is right-aligned. Confirmation, activity, tool summaries, result, and cleanup cards are left-aligned beneath a header that names the run and its start time. Connector-authored safe summaries may name the tool and count returned records. They must not expose raw arguments or provider output. Cross-Bot messages and routine-created notices do not appear in the first release.

The composer contains one multiline prompt and `Review task`. It has no attachment, microphone, mention, or schedule control. It stays disabled until one active grant passes current connector checks. It also stays disabled while that Bot has a live confirmation, a nonterminal run, or any incomplete cleanup obligation. The first release permits one live confirmation and one occupied run slot per Bot. Its empty state explains the difference between a connection and a Bot grant.

Submitting a prompt creates a five-minute confirmation, not a run. The confirmation page shows changed or expired state plainly and sends the user back to the Bot instead of silently recompiling.
...
`BotListItemV1` contains `bot_id`, `name`, `title`, `palette_color_id`, `active_revision_id`, `presentation_status`, `last_activity_at`, and `usable_grant_count`. It contains no prompt or result excerpt.

Status uses this precedence: `Outcome unknown`, `Cleanup required`, `Cleaning up`, `Cancelling`, `Running`, `Needs confirmation`, `Needs configuration`, `Needs access`, then `Cancelled`, `Failed`, or `Completed` from the latest run. `Needs configuration` means a selected tool policy, skill revision, connector revision, or model route is no longer usable. A Bot with no prior run and usable configuration and access has `Ready`. `PresentationStatusV1` maps every run, cleanup, evidence, catalog, grant, and confirmation state. An exhaustive TypeScript switch test fails when a new state has no user-facing mapping.
...
Page handlers serialize allowlisted `BotRosterViewV1`, `BotWorkspaceViewV1`, `RunConfirmationViewV1`, `RunDetailViewV1`, `RunPollPageV1`, `BotAccessViewV1`, `ConnectionDetailViewV1`, `CatalogViewV1`, and `AuditEventViewV1` schemas. Each view carries server-derived `available_actions` and stable denial reasons. Exact-key response tests seed encrypted and provider reference columns and prove that no page or JSON response includes them.

Roster state and composer enablement are hints. `createRunConfirmation` and `confirmAndCreateRun` reload the catalog, grant, authorization, cleanup, and slot authority. They return `grant_not_active`, `bot_busy`, `cleanup_blocked`, or `confirmation_stale` before external work. No mutation predicate reads `bot_activity` or `usable_grant_count`.
...
Final answers render as plain text with `white-space: pre-wrap`; the first release does not render model Markdown or HTML. `SourceReferenceV1` is connector-produced and contains provenance, a safe label, displayed hostname, and an `https:` URL whose host is on that connector's allowlist. Strip bearer and tracking query parameters. Model-authored citations remain unlinked text. External links use `rel="noopener noreferrer"` and `Referrer-Policy: no-referrer`. Browser code writes text with `textContent`, not `innerHTML`.

## Organization catalog and Bot assignment

The database calls the tenant boundary `account`; the product calls it an organization. The initial `owner` is the organization administrator. Additional roles and members remain deferred, but the schema and route names must not imply that a personal login owns reusable provider authority.

The organization administrator curates two catalogs:

- An organization tool policy selects a pinned Metorial deployment version and named connector tool key. Its create command accepts those identifiers only. Reviewed connector code reloads the pinned vendor schema, effect classification, data rules, resource-mapping support, and model-disclosure rule, then stores those server-derived bytes and digests. An owner cannot submit or relabel security claims. A classification change requires a new connector release and policy revision.
- A skill is a versioned instruction module. `SkillRevisionV1` contains a name, purpose, encrypted instruction content ID, bounded input and output schemas, requested organization tool policy IDs, maximum instruction bytes, author, plaintext digest, and lifecycle state. Its schema has no code, credential, package, URL, MCP, or executable-template fields. Instruction text is untrusted and may still contain a URL, secret, hostile instruction, or sensitive organization data. A secret and URL scanner can warn or deny known patterns but cannot prove the text safe. The confirmation names the exact revision and says its bytes go to the model provider. The compiler places it below immutable system policy and never interpolates it into authorization, a network address, or a vendor credential.

A Bot revision selects organization tool policies and skill revisions. Selection is intent, not authority. All selected tool policies must resolve to the same reviewed connector deployment, provider authorization, and capability grant. A run tool is permitted only when it appears in the active organization policy, the Bot revision, the connector contract, the capability grant, and the Metorial session filter. A skill cannot add a tool, data class, destination, mount, or limit. The signed manifest includes each skill revision ID and plaintext digest. The encrypted runtime envelope carries the exact instruction bytes. Compilation denies an unresolved, disabled, cross-deployment, or cross-authorization dependency.
...
| `GET /bots?q=&cursor=&limit=` | Attention-sorted Bot roster and `New Bot` control. `q` matches Bot name and title only. |
| `GET /bots/new` | Name, title, palette color, job, standing instructions, organization tools, and declarative skills. |
...
| `GET /bots/:botId/access` | Connections, active and expired grants, tool allowlist, resource mapping, destinations, limits, purpose, and revocation controls. |
...
| Surface | Routes added with the artifact workspace |
|---|---|
...
| Tool availability | Signed manifest and Metorial filter | Capability gateway checks the manifest, then Metorial applies its tool allowlist |
...
| Active execution sequence | One run-owned Durable Object | Serialized requests and local SQLite state |
...
| Provider business-data writes | Nobody | The gateway rejects tools with known or possible business-data mutation. Connector review discloses quota, logs, access timestamps, and other incidental vendor effects. Unknown incidental effects deny the connector. |
| Audit evidence | OpenBot control store | Append-only application records with a hash chain |

The model, user prompt, skill text, tool output, artifact body, MCP server, and downstream model provider are untrusted. The control-plane Worker, orchestrator Worker, capability gateway, artifact gateway, runtime Worker, deployment configuration, and their secrets are trusted enforcement code.
...
| `orchestrator` | No public route | `CONTROL_DB_FRESH`, Queue consumer, Metorial and OpenRouter management keys, runtime DO namespace, manifest signing key, auth-config keyring, vendor-capability keyring, content and configuration keyrings, runtime-envelope keyring, keyed-digest key | Recompile and sign manifests, create runs, provision and clean up vendor resources, drive run objects, import results |
| `capability-gateway` | Private service binding only | `CONTROL_DB_FRESH`, manifest public keyring, vendor-capability keyring, keyed-digest key, outbound Metorial and OpenRouter clients, private artifact run service after workspace adoption | Atomically reserve each call, then issue one constrained model, tool, or artifact request |
...
Deploy in this order: artifact gateway when enabled, capability gateway, runtime, orchestrator, control plane. The orchestrator binds the runtime DO namespace with `script_name`. Every Wrangler environment repeats bindings because non-inheritable bindings do not flow into environment blocks.

The public control plane cannot sign a manifest or decrypt auth-config and vendor-capability records. A private orchestrator RPC reloads current policy inputs, compiles the manifest, and signs only those bytes. The runtime has no global database, vendor key, vendor URL, browser session, management binding, shell, filesystem, or public route. It receives a random 256-bit gateway token for one run. The gateway stores only its hash and binds it to the run, DO ID, manifest digest, audience, issue time, and expiry.
...
| Run-attempt lease and heartbeat | 180 seconds and 30 seconds |
...
The gateway sends one literal model ID and `provider.order` with one provider slug. It sets `allow_fallbacks: false`, `require_parameters: true`, `data_collection: "deny"`, `zdr: true`, `parallel_tool_calls: false`, and explicit token limits. If the selected model and provider cannot satisfy ZDR, the run is denied. Do not use model aliases, model arrays, auto routing, plugins, or provider fallback.
...
`account_membership(account_id, user_id, role)` is the only route from identity to account authority. The initial schema permits only `owner`; `member` waits for the team permission model. Removing an owner, revoking a capability grant, expiring a grant, or revoking a provider authorization atomically advances a revocation fence, marks affected active runs cancellation-requested, and creates cancellation and cleanup outbox records. The gateway checks both the run and that fence.
...
Every ciphertext includes `kid`, encryption domain, format version, random 96-bit nonce, and additional authenticated data with account ID, record ID, field type, and schema version. Cross-domain decryption must fail. Tests prove that the control plane cannot decrypt vendor capabilities or artifact metadata, the capability gateway cannot decrypt prompts, configuration content, auth-config refs, or artifact metadata, the artifact gateway cannot decrypt prompts, configuration content, or vendor capabilities, and the runtime cannot decrypt stored content. Rotation keeps old decrypt or verify keys during a documented overlap, rewraps stored records, then retires the old key. Losing an active key is a recovery event, not a silent reset.
...
| `account`, `account_membership` | Account is the organization boundary; unique membership on account and user; initial role is owner only | Removal advances the account revocation fence and cancels active runs |
| `organization_tool_policy` | Server-derived pinned deployment version, connector tool key, schema digest, effects, data rules, reviewer, version, dependency revocation fence | Active or disabled; disabling invalidates confirmations and cancels dependent runs only |
| `skill`, `skill_revision`, `configuration_blob` | Append-only declarative metadata, encrypted instruction content ID, requested organization tool policies, bounded schemas, author, plaintext digest, current-default pointer, dependency revocation fence | A revision is active or disabled; referenced encrypted content remains; creating a revision does not alter existing Bot revisions |
| `bot` | Account, name, title, fixed palette ID, generated initials, NFKC-lowercase search fields, current revision, cosmetic version | Cosmetic changes update the bot; executable changes do not |
| `bot_revision`, `bot_tool_selection`, `bot_skill_selection` | Append-only `BotRevisionV1`: job, standing instructions, prompt-template version, selected organization tools and skills, connector revision, model route, limits, outbound-data rules, digest | A bot points to one current revision; any executable change creates a revision |
...
| `capability_grant`, `capability_grant_revision`, `policy_revision` | Account, bot revision, connection, tools, data rules, limits, purpose, expiry, revocation fence | Revisions append; grants move from active to revoked or expired |
...
| `run` | Unique account-scoped idempotency key; accepted disclosure digest; manifest digest; opaque DO ID and jurisdiction; content-erasure version | Requested, queued, provisioning, running, result_pending_import, result_imported, succeeded, failed, cancellation_requested, cancelled, cancelled_with_effect_unknown, outcome_unknown, result_lost_after_execution |
...
| `run_attempt` | Run, attempt, 180-second lease, 30-second heartbeat, owner, monotonically increasing fence | Active, completed, abandoned, outcome_unknown |
...
| `run_capability` | Token hash, run, DO ID, manifest digest, audience, expiry, next sequence, reserved counters, revocation fence | Active, revoked, expired |
| `gateway_call` | Unique run and sequence; kind, tool or model key, request digest, declared reservation, fence, timestamps | Pending, completed, outcome_unknown, denied |
...
| `cleanup_obligation` | Vendor target, lease, attempts, last error | Pending, running, complete, manual_required |
...
`POST /runs` sends `confirmation_id` to `confirmAndCreateRun`. The orchestrator reloads authority, verifies the current session binding, allocates the DO ID, recompiles, and denies if the prompt, revision, skill, tool, destination, mount, limit, model, or canonical bytes changed. The guarded batch consumes the confirmation, clears its confirmation slot, links the stable prompt content ID through `run_content`, claims the run slot, and binds the disclosure digest to the signed manifest and run. A concurrent repeat returns the recorded run for the same idempotency command or `confirmation_consumed` for another command; it never creates a second run. Discard and expiry release only a matching confirmation ID and erase its blob ciphertext when no run references it.
...
`discardRunConfirmation`, `expireRunConfirmation`, `terminalizeRunAndMaybeReleaseBotSlot`, and `completeCleanupAndMaybeReleaseBotSlot` use the account, Bot, matching confirmation or run ID, and expected slot version. A stale operation is a no-op when the slot points elsewhere. Terminalization releases a slot only when the run created no vendor resource and has no cleanup obligation. Otherwise only observed cleanup completion releases it. Bot creation creates its slot in the same transaction.
...
Use strict ESM TypeScript, pnpm workspaces, project references, Zod at every trust boundary, Hono, `@hono/zod-openapi`, Better Auth, Drizzle, Prettier, Vitest, fast-check, `@cloudflare/vitest-pool-workers`, Testcontainers, and Playwright. Generate and commit the `/api/v1` OpenAPI document. Do not add Turborepo until task timing shows a need.

Each database package exports OpenBot repositories and an opaque `createAuthStorage()` result for the Better Auth composition root. No route receives a Drizzle handle. Only database packages import Drizzle drivers. Only the orchestrator imports vendor management clients. Only the capability gateway imports vendor data-plane clients. Only the artifact gateway receives an R2 binding or imports artifact body storage. Only the runtime defines a Durable Object class or imports its storage APIs; the orchestrator imports the typed namespace client.

The profile entry files are composition roots. A bundle contains one database driver only. Required scripts are `dev`, `build`, `typecheck`, `test`, `test:integration`, `test:e2e`, `format`, `format:check`, `cf-typegen`, `db:generate:<profile>`, `db:migrate:<profile>:local`, `db:migrate:<profile>:remote`, `deploy:<profile>:<env>`, `tail:<env>`, `verify`, `verify:integration`, and `verify:preview`. `cf-typegen` runs pinned `wrangler types` for every config and CI fails on a dirty generated-type diff.
...
| D1 guarded create | Both legal create-versus-revoke histories under two writers | D1 schema freeze | Change the storage design |
| Gateway reservation | Concurrent duplicate sequences produce one outbound request and one spent reservation | D1 schema freeze | Change the store protocol |
...
Item 2 is not done until the repository contains literal values for the first connector and route:
...
Check a clean install, formatting, types, unit tests, Worker dry-runs, forbidden imports, a seeded secret, an intentionally unpinned dependency, and the core route fixture. `core-routes.json` enumerates the HTML, form-action, JSON, operator, auth, and internal routes registered for the read-only core. A separate artifact-prefix deny fixture asserts that no artifact route is registered. Item 11 creates the authoritative `artifact-routes.json` only after its command matrix passes. `pnpm verify` must reproduce the hermetic CI job.

Done when the D1 control-plane shell, fake orchestrator, fake gateway, and fake runtime compile with no vendor credential. Bundle inspection must show only the D1 driver. PostgreSQL, MySQL, and artifact entrypoints do not exist yet.

### 2. Settle D1 contracts and blocking gates

Create the ADRs, `docs/threat-model.md`, logical Zod contracts, state transitions, error codes, and disposable vendor and platform probes first. Add a command matrix that names every core mutation's route, actor, idempotency scope, expected version, success response, repeat response, stale response, audit event, and outbox effect. Define the organization catalog, declarative skills, Bot selection, live-confirmation slot, run slot, and a versioned optional manifest-extension envelope before freezing D1 tables. Do not add artifact tables yet. Run the core shared-protocol gates and fix the literal connector, route, and jurisdiction values. Only then write the D1 Drizzle schema, committed migrations, native DO migrations, guarded-run SQL, and gateway-reservation SQL. The threat model names each asset, attacker, entry point, control, residual risk, test, and operator-visible failure state. Implement only D1. PostgreSQL and MySQL remain contract targets, not code yet.

Check empty migration, fixture upgrade, both deployed D1 create/revoke histories, two-writer gateway reservation, cross-account foreign keys, duplicate idempotency keys, audit-head contention, DO migration and tombstone replay, Queue send-before-mark crash, DLQ replay, and every numeric limit. Add direct contracts for one live confirmation, concurrent slot claim, run-bound slot release, status projection at a fixed `as_of_ms`, normalized search, cursor stability, poll views, dependency-specific catalog fences, skill dependency denial, cross-deployment denial, and an unrelated Bot that remains runnable after another catalog entry is disabled.
...
Create `OrganizationToolPolicyV1`, `SkillRevisionV1`, `BotRevisionV1`, `DisclosureSnapshotV1`, the versioned manifest-extension envelope, the pure policy compiler, connector review format, private manifest signing RPC, keyrings, envelope formats, redaction library, keyed digests, audit append and verifier, `policy explain`, and `manifest verify`. Add the session-bound preview and one-use confirmation contracts without a browser UI.

Check that narrower inputs never broaden a manifest. Mutating any signed field must fail. Expired grants, disabled catalog dependencies, disabled skills, tools absent from the Bot selection, cross-deployment or cross-authorization policies, write-capable tools, unsupported scopes, disallowed outbound fields, and unavailable ZDR must return stable deny codes. Test confirmation expiry, discard, session mismatch, tab replay, two concurrent submissions, changed prompt, skill, provider, connector schema, destination, and limit. Run canonical JSON and crypto vectors in Node and Workers. Negative key tests must fail across every encryption domain.
...
### 4. Prove a D1 walking skeleton with all fakes

Use a fixed test identity and seeded organization owner only in local and test builds. Seed one organization tool policy and one declarative skill selected by one Bot. Add the shared two-column shell, `GET /bots`, `GET /bots/:botId`, `GET /run-confirmations/:confirmationId`, and `GET /bots/:botId/runs/:runId`. Add `POST /run-confirmations`, discard, `POST /runs`, guarded run creation, outbox dispatch, fake provisioning, fake gateway reservations, fake runtime byte frames, terminal import, acknowledgement, cleanup retry, and the result view. Skip general CRUD, OAuth, artifact bodies, and polished progress UI here.

Check the happy path, no grant, missing or changed confirmation, one-live-confirmation enforcement, two starts for one Bot, starts for two Bots, stale slot release, both stale-grant histories, duplicate Queue delivery, concurrent gateway calls, crash at each fault point, disconnect and commit failure around final-result import, final-result encryption, and cleanup status separate from execution status. Seed every presentation status and 51 Bots for pagination, search, and stable-cursor tests. At 1440 by 900 pixels, assert the roster, task feed, confirmation, and result are visible in the intended hierarchy. At 390 by 844 pixels, assert the roster becomes a drawer and the task form remains usable.

Done when one Playwright test starts a run and reads the result, while the deny cases make zero fake external calls.

### 5. Add real identity and the control-plane UI

Add the audited operator-token endpoint and admin commands, Better Auth D1 storage, database rate buckets, closed registration, owner membership, and the initial browser and JSON route tables in this plan. Build the organization tool and skill catalogs, bot roster and workspace, profile and revision history, connection setup, access and revocation, run confirmation, run detail, cleanup obligation, audit, and read-only settings pages. Add two-second cursor-based run polling, security headers, CSRF, confirmation-content sweeping, and retention deletion. Each poll reads imported global events; SSE is deferred. Every HTML and JSON route calls one shared use case.
...
### 8. Replace the runtime fake with the raw Durable Object

Add the private DO export, jurisdiction-aware `newUniqueId()` allocation, native migrations, signed start and cancel commands, in-memory prompt decryption, manual model loop, NDJSON byte stream, call-boundary journal, result-import acknowledgement, restart-to-unknown behavior, separate scrub and delete commands, and public-route denial.

Check the wrong DO ID, bad signature, wrong audience, expiry, nonce replay, altered prompt ciphertext, extra tool, changed schema, prompt injection asking for new tools or fetch, invalid NDJSON, bytes after terminal, all size and count limits, disconnect, cancellation during every external call, local event replay, Queue redelivery at the retention boundary, result loss before acknowledgement, secret-sentinel scans of commands, RPC errors, SQL, logs, event frames, and DLQ bodies, sensitive scrub, and final deletion.

Done when one real read-only run completes through the gateway, Metorial, and OpenRouter. Restart never repeats an uncertain request.

### 9. Add PostgreSQL, then MySQL

Implement the PostgreSQL core schema, Better Auth schema, migrations, repositories, request-scoped auth factory, invocation-scoped Hyperdrive adapter, distinct core roles, profile entry roots, and backup guide. Run the unchanged core repository, policy, auth, gateway-reservation, organization catalog, roster, concurrency, audit, Queue, and Playwright suites. Repeat for MySQL only after PostgreSQL passes. Artifact entrypoints and roles still do not exist.
...
Check a clean install in a fresh Cloudflare account, deployment order, absent public runtime routes, vendor outages, Queue backlog, DLQ recovery, manual cleanup, D1 Time Travel restore, external database restore, key rotation, retention deletion, dependency licenses, preview promotion, and previous-code rollback against the expanded schema.

Done when an operator can deploy, run, inspect, cancel, clean up, back up, restore, rotate keys, and verify the read-only slice without an OpenBot-operated service or an R2 bucket.
...
Extend the capability gateway so the model can propose an action. OpenBot must canonicalize connector arguments and targets, compute a digest, store the proposal, collect explicit human approval, and rebuild the provider call from the approved bytes. The runtime never receives a write tool.

Check argument, resource, and schema substitution; approval replay and expiry; revoke while waiting; concurrent dispatch; provider idempotency; crash uncertainty; and evidence that the provider received the approved bytes.
...
- OpenCode, Agents SDK, Code Mode, Sandbox, Cloudflare Browser Rendering, shell, package installation, executable skill packages, subagents, and arbitrary MCP servers
- Persistent conversation context, replies, reactions, group chats, direct Bot-to-Bot messages, task handoffs, learned memory, schedules, unattended runs, additional organization roles, teams, shared provider authorizations, and a marketplace
...
- Inline provider-write approval cards, `Always allow`, and any UI that implies provider business data can change before the broker in item 12 passes
...
Source snapshot checked on 2026-08-21. Product behavior is treated as research input, not as an API or compatibility contract.
