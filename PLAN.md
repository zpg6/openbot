...
The same private operator package has a pure compiler for an untrusted, run-specific Worker artifact candidate covering the sink, Writer A, and Writer B. It binds exact module hashes, canonical direct Versions Upload metadata hashes, fixed multipart part order, role-selected entrypoints, generated names, the initialized D1 ID, both Writer-to-sink bindings, compatibility and toolchain pins, and package and build-recipe digests. Build fields are untrusted caller claims, not proof of remote configuration. The checked-in Writer A and Writer B sources now provide separate role-only entry modules and Access-aware default fetch handlers. It rejects source maps and workstation paths in module text. It rejects build claims for assets, extra modules, upload-metadata variables or secrets, tails, logging, observability, and routes. Its `workers_dev`, preview URL, route, export, ingress, and runtime-version fields are claimed local observations only. The candidate uses the current direct-API D1 binding field `database_id`; it does not claim byte identity with Wrangler 4.125.0, which normalizes that config field to `id`. Local dry-run bundles still emit source maps, use synthetic build-time HTTP configuration, and lack deployed runtime-version evidence, so they do not pass. Unit tests simulate successful verified Access contexts. Workerd covers distinct role-only RPC modules and unauthenticated fetch denial, not deployed Access routing. The output exposes no raw upload bytes, is never upload-eligible or deployment-ready, and cannot advance lifecycle or gate state. No upload adapter exists. Versions Upload and deployment require separate lifecycle, reconciliation, readback, and cleanup work.
...
The isolated canary also does not repair the existing production-path incompatibility: the artifact candidate describes classic multipart metadata and emits a bare role-and-caller-bound binding digest, the pure protocol describes beta JSON modules and emits a differently domain-separated `sha256:` binding digest without an artifact digest, and lifecycle evidence requires bare matching artifact and binding digests. Production upload remains blocked until one reviewed serialization and one canonical artifact-and-binding digest chain feed upload, readback, deployment, cleanup, and opaque lifecycle evidence. A credentialed canary adapter and a later production adapter must remain separate.
