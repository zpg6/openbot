...
The lifecycle requires the opaque initialized-database result after `database_created`, then splits Worker Version upload from 100 percent Deployment for the sink, Writer A, and Writer B. Each upload binds the artifact and binding-configuration digests. Every deployment binds those same digests, is fixed to one exact uploaded version at 100 percent traffic, and requires `force: false`. The generic journal function cannot advance any of the six mutation steps, and the plain journal schema rejects every fabricated completed Worker-mutation prefix. A future consumer must hold opaque upload or deployment evidence. A database-bootstrap failure before dispatch has its own `pre_dispatch` semantic. Any post-dispatch upload or deployment failure has a step-specific ambiguity reason, an unknown outcome, no retry, and required manual cleanup. Upload remains blocked on a missing script-shell and subdomain-settings protocol. Direct Versions Upload metadata cannot prove that a new script will not expose a public Versioned Preview URL, and a per-script settings readback needs an existing script. No upload or deployment adapter, evidence consumer, or gate promotion path exists.
