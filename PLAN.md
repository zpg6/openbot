...
| `bot_revision`, `bot_tool_selection`, `bot_skill_selection` | Append-only `BotRevisionV1`: job and standing-instruction data classes, prompt-template version, selected organization tools and skills, connector revision, model route, limits, outbound-data rules, digest | A bot points to one current revision; any executable change creates a revision |
