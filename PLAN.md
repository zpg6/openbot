...
A Bot revision selects organization tool policies, skill revisions, and zero or one organization compute policy. Selection is intent, not authority. All selected provider-tool policies must resolve to the same reviewed connector deployment, provider authorization, and capability grant. A provider tool is permitted only when it appears in the active organization policy, the Bot revision, the connector contract, the capability grant, and the Metorial session filter. Code is permitted only when the installation profile, organization compute policy, Bot revision, current compute grant, disclosure, signed manifest, run capability, capacity reservation, JavaScript runner, and code-call reservation agree. A skill cannot add a provider tool, compute policy, data class, destination, mount, or limit. The signed manifest includes each skill revision ID and plaintext digest plus the compute-policy and profile digests when selected. The encrypted runtime envelope carries the exact instruction bytes. Compilation denies an unresolved, disabled, cross-deployment, cross-authorization, compute-policy, compute-grant, or code-profile dependency. User-authored prompts, jobs, standing instructions, and skill instructions default to the `organization` data class. A trusted server-side review may raise them to `restricted`; public commands cannot submit or lower the classification. `unknown` denies model or code dispatch.
...
| `bot_revision`, `bot_tool_selection`, `bot_skill_selection`, `bot_compute_selection` | Append-only `BotRevisionV1`: job and standing-instruction data classes, prompt-template version, selected organization tools and skills, optional compute policy, connector revision, model route, limits, outbound-data rules, digest | A bot points to one current revision; any executable change creates a revision |
