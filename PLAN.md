...
Before any remote effect, a separate immutable private cleanup-obligation record can bind the prepared operation, execution commitment, and independently digested cleanup grace. Its read-only verifier changes no file. The record is caller-constructible, the runner does not write or consume it, and effect claims do not yet carry its digest, so it authorizes no cleanup.
