...
## Base layer before product work

The MVP hardening order is now explicit. Product-facing work waits until the base layer can survive interruption, ambiguity, and restart without borrowing authority from caller assertions.

- Durable identities bind each operation, attempt, remote resource, and stored record.
- Append-only state, effect, and encrypted-archive histories preserve every transition and publication edge.
- Lease epochs fence each effect and require current-owner reassertion.
- Composed dispatch-response sessions bind pre-dispatch intent, dispatch start, bounded response capture, archive publication, and terminal claim append.
- Tri-store consistency compares operation state, effect claims, and archive inventory without repairing any store.
- Durable transcript reconstruction joins the effect journal to encrypted archive inventory after restart and reproduces the redacted request history without raw paths or bodies.
- Recovery reducers turn each consistent or one-store-ahead state into a fixed safe action or a manual stop.
- Cleanup obligations are created with the first possible remote effect and remain visible until exact absence is observed.
- Authority stays separate from local durability. A valid local chain, lease, archive, inventory, or recovery result cannot authenticate Cloudflare or authorize lifecycle and gate changes.

This order applies across the repository, not only to the Worker API canary. A UI, live command, or broader integration does not move ahead while its durable identity, interruption recovery, cleanup, and authority boundaries remain undefined.

The Worker canary now uses one execution-nonce commitment across operation, lease, and effect records. Every effect claim carries the exact lease generation and canonical lease-record digest that owned it. The read-only checker double-samples operation, effect, and lease histories, checks every historical claim against its exact operation revision and lease epoch, and rejects missing, changed, expired-at-dispatch, or unstable bindings. A separate read-only classifier double-samples that snapshot with the encrypted-archive inventory and distinguishes aligned histories, intent-only abandonment prerequisites, ambiguous effects that must not be replayed, missing archives, and archive-ahead states. A durable transcript reader double-samples the same histories, validates every effect claim, joins each terminal response to its exact archive record, and reconstructs redacted request entries after restart. It exposes digests and timing claims, never raw paths or bodies. The keyed archive-ahead reducer checks the active lease and unchanged local heads, verifies AES-GCM and the plaintext digest without returning the plaintext, and appends only the exact missing `response_observed` claim. A takeover lease may authorize that local repair, but the terminal claim retains the earlier lease epoch that owned the dispatch. The reducer sends no request and cannot transition operation state or grant cleanup, lifecycle, or gate authority. The runner consumes none of these contracts, and their outputs remain locally forgeable and non-authoritative.

Before any remote effect, a crash-aware bootstrap creates or reuses the exact prepared operation state and immutable private cleanup-obligation record, rejects prior effect or archive history, acquires the cooperative lease, and reasserts all local heads. A private durable-driver session now owns the returned lease identity and composes each exact-identity operation with the joined response-claim hooks without exposing the lease owner, raw execution nonce, script name, ownership tag, attempt tag, operation record, or cleanup-obligation record. The obligation binds the prepared operation, execution commitment, and independently digested cleanup grace. Its read-only verifier changes no file and scopes concurrent snapshots to the exact plan and execution stem. Cleanup dispatch, response, encrypted archive, inventory, consistency, and recovery contracts carry and reassert that exact obligation digest; forward claims must keep it null. The caller still supplies the operation, workflow step, and archive key and can retain them. The session prepares no request, sends no network call, releases no lease, and grants no dispatch or cleanup authority. The credentialed runner does not consume it, and none of these caller-constructible local records authenticates a remote effect.
...
|---|---|---|
| Durable identity and state | The private durable-driver session establishes exact prepared state, cleanup obligation, empty histories, and current lease, retains the lease owner in its closure, and accepts only the same immutable execution identity | Make the credentialed runner use this session as its only bootstrap and claim-composition path, then define takeover and release rules across restart |
| Dispatch and response recording | Each durable request session records intent and start, captures bounded bytes, publishes the encrypted archive, then appends the terminal claim without exposing lease ownership | Preserve the exact session across runner interruption and forbid every direct credentialed use of the lower-level bootstrap and claim factories |
| Recovery | Read-only consistency, durable transcript reconstruction, base classification, and keyed archive-ahead reconciliation cover exact, ambiguous, missing-archive, and archive-ahead histories | Add driver takeover rules, post-crash cleanup resumption, and terminal state transitions for every request step |
...
The local E2E workload defaults to three operations at concurrency two. Each operation opens the durable-driver session and creates all response-claim hooks through it; the benchmark no longer imports or manually joins the bootstrap lease owner and response-claim factory. It writes four state revisions, nine effect claims, three encrypted response archives, and one cleanup obligation, then reaches the durable `delete_dispatching` fence without issuing a DELETE. It reconstructs the three-entry durable transcript and compares every method, sequence, request digest, path digest, status, and response digest with the live transport transcript. `OPENBOT_BASE_E2E_OPERATIONS`, `OPENBOT_BASE_E2E_CONCURRENCY`, and `OPENBOT_BASE_E2E_RESPONSE_BYTES` raise the workload to 128 operations, concurrency 16, and 256 KiB per synthetic response. A second workload races up to 64 writers for the same revision-zero state path and requires one winner. The benchmark reports completed bootstraps, throughput, and latency percentiles, but CI asserts record counts, bindings, transcript equality, recovery classification, and CAS outcomes rather than wall-clock limits.

Scaled runs have found two real false-contention bugs. Archive inventory snapshots included directory timestamps and link counts, and cleanup-obligation reads compared the entire shared directory. Another plan publishing an unrelated record could therefore make a valid exact read unstable. Archive inventory now snapshots only names with the requested plan prefix, double-reads that set, rechecks every selected file, and compares the directory's device, inode, and mode. Cleanup-obligation reads compare only the exact plan and execution stem. Unrelated plans no longer invalidate either read; a same-plan change still fails closed.
...
Before schema freeze, run both deployed D1 create/revoke histories, two-writer model, provider-tool, and code reservations, Sandbox-capacity contention, and audit-head contention against disposable probe tables. `packages/d1-probes` exercises the guarded D1 statement patterns under local workerd to catch syntax, rollback, and error-mapping defects. Its observations are always `hermetic_test_only`, set `eligible_for_attestation` to `false`, and cannot enter application packages. `packages/d1-probe-operator` is a private Node-only preflight boundary. `pnpm d1-probe:plan` reads one canonical request from standard input and an HMAC key from file descriptor 3, emits only a non-authoritative plan, and performs no network or deployment action. The request supplies distinct suffixes generated by the operator with a cryptographically secure random source; the compiler validates their shape and uniqueness but does not claim to prove their entropy. It also supplies one canonical HTTPS DNS origin with no path, port, credentials, query, or fragment and one database jurisdiction. The plan derives three distinct HTTPS path routes under `/_openbot-d1-probe/`, binds each route to its exact Writer or readback script and method, and derives one Access application path covering only that prefix. It HMAC-commits the origin, Access path, route patterns, real account and zone identifiers, each operator-denied production database ID, and every generated resource name. Before later deployment code may resolve those raw identities, it must recompile the complete plan from the parsed request and HMAC key and compare the canonical plan bytes. Only that path creates the opaque in-memory verified preflight context. A copied object cannot construct the context, its resolved request and plan are deeply frozen, and the HMAC key is neither retained nor serialized. A separate readback inspector accepts only that opaque context. It checks a strict projection of Cloudflare's zone and DNS responses against the exact account, zone, and probe hostname. It requires an active, unpaused full zone, an exact-name query for proxied DNS, complete pagination, unique records, and only proxied A, AAAA, or CNAME records. The credentialed reader performs the two fixed Cloudflare API reads with a zone-scoped read token, no redirects or retries, bounded responses, and one total deadline. `pnpm d1-probe:check-route` accepts no arguments, reads the canonical request-and-plan envelope from standard input, the HMAC key only from dedicated file descriptor 3, and the API token only from dedicated file descriptor 4. It ignores environment credentials, recompiles the plan before the network read, and emits only the canonical non-authoritative inspection or a fixed denial code. It performs no mutation or deployment and cannot authorize a gate. The unwired D1 creation adapter accepts only the opaque observation from that live reader. It rechecks the plan, HMAC key, and exact planned journal before one create request. The body binds the disposable name, `read_replication.mode` equal to `auto`, and the selected jurisdiction. The adapter follows no redirect, retries nothing, HMAC-commits the returned ID, and exposes that raw ID only through an opaque in-memory cleanup target. A mismatched resource or production-ID collision becomes `manual_required` with that target. A lost or unprovable response becomes `manual_required` without retry. The matching unwired delete adapter accepts only that target and either the exact normal cleanup prefix or its target-bound create-or-bootstrap-failure journal. It rechecks the plan, key, ID, and name before one delete request. Dispatch makes the target one-use even when the response is lost. A successful wrapper advances only `database_deleted` and records `sdk_acknowledged`, never verified absence. An ambiguous response becomes `manual_required` without retry, and emergency acknowledgement does not clear manual review. The unwired read adapter then searches Cloudflare's documented D1 list endpoint with the exact generated name. It requires the same target, a delete dispatch outcome, the matching journal, the key, and a D1 Read token. It checks complete bounded pagination and rejects the exact returned ID or name as still present. A clean search records `control_plane_absence_observed` and mints an opaque context bound to the target object, plan, journal, ID and name commitments, delete outcome, and observation digest. Forged or copied objects do not resolve. The matcher rejects another target or changed journal. The observation does not independently prove deletion, advance `all_resource_absence_confirmed`, clear manual review, or finish cleanup. The generic lifecycle function returns `evidence_required` for that final step. A future aggregate cleanup operation must consume the exact opaque observation together with equivalent observations for every other resource and enforce one-use. That consumer does not exist. Safe reads may be invoked again after failure, but the adapter performs no automatic retry. No root command invokes any mutation or absence adapter yet. The lifecycle journal advances only in the fixed create and cleanup order, binds returned IDs and names, rejects a denied database before provisioning, and makes ambiguous state terminal for manual review. `packages/d1-probe-rpc` defines the private writer-to-sink receipt request, the digest-bound gateway reservation, and the one-use gateway trial request. The sink inserts a fresh receipt for every valid request and has no deduplication key. Writer A and Writer B expose separate role-pinned entrypoints. Their local-only trial operation records one assigned child and GO receipt, polls an exact two-child ready set through fresh primary sessions, then enters the guarded gateway reservation. It validates every returned row and metadata field and calls the sink once only after commit. Exact replay and changed-digest denial require fresh primary readback. Missing assignments and closed trials deny separately. Missing bookmarks, ambiguous D1 state, barrier read failure, and missing private-RPC responses become `outcome_unknown`; no such path retries the sink. A barrier timeout makes no sink call. Checked-in Wrangler files are local-only, have no route, and exist for dry-run binding checks. The deployed driver must verify that the origin belongs to the supplied zone and has the required proxied DNS before it creates the exact routes. It must generate the real names, database binding, routes, Access resources, operator child processes, collector, and cleanup from the verified preflight context. Operator and probe packages cannot enter product code. ADR 0009 and `docs/fixtures/d1-concurrency-probe.json` define the deployed test. That test uses two distinct writer deployments, a private non-deduplicating sink, fresh `first-primary` readback, and no automatic application retry after ambiguity. Its untrusted report must pass the pure gate-specific assessment against operator-supplied expected context before offline review and signing. The assessment is not an attestation and cannot promote either D1 gate. After promotion, check the empty migration, fixture upgrade, cross-account foreign keys, duplicate idempotency keys, DO migration and tombstone replay, Queue send-before-mark crash, DLQ replay, and every numeric limit. Add direct contracts for one live confirmation, concurrent slot claim, run-bound slot release, Sandbox destroy-bound capacity release, status projection at a fixed `as_of_ms`, normalized search, cursor stability, poll views, dependency-specific catalog fences, skill dependency denial, code-profile denial, cross-deployment denial, and an unrelated Bot that remains runnable after another catalog entry is disabled.

`production_database_denied` means Cloudflare returned a newly created database ID that matched the operator deny-list. OpenBot blocks every later provisioning step and retains an opaque cleanup target. It does not claim that the create request had no effect.

Database bootstrap uses the pinned disposable schema at `@openbot/d1-probe-rpc/schema`, shared with the local workerd probes. An unwired adapter accepts only the opaque created database and exact `database_created` journal prefix. It submits no caller SQL. The first batch requires an empty probe namespace, installs the exact 28 tables and two triggers, and reads the namespace back across tables, triggers, indexes, and views. It ignores unrelated Cloudflare-reserved storage objects but rejects an unprefixed object attached to a probe table. Three fixed follow-up requests verify `PRAGMA foreign_keys = 1` and an intentional rollback canary under the same 20-second deadline. Success mints opaque initialized-database evidence without advancing `sink_private_shell_created`. An ambiguous or contradictory result marks the exact target for manual cleanup without a mutation retry. No root command invokes this adapter, and its output cannot attest or promote a gate.

The same private operator package has a pure compiler for an untrusted, run-specific Worker artifact candidate covering the sink, Writer A, and Writer B. One shared `beta_worker_json_version_v1` contract now derives the canonical beta JSON Version body, exact module hash, role-and-caller-bound binding-configuration digest, artifact digest, and `deploy=false` Version request digest. The artifact candidate and provisioning protocol consume that same projection for all three roles, and lifecycle upload and Deployment observations require the same bare digests. Deployment observations additionally bind a separate canonical no-force Deployment-request digest. The candidate also binds role-selected entrypoints, generated names, the initialized D1 ID, both Writer-to-sink bindings, compatibility and toolchain pins, and package and build-recipe digests. Build fields are untrusted caller claims, not proof of remote configuration. The checked-in Writer A and Writer B sources provide separate role-only entry modules and Access-aware default fetch handlers. The compiler rejects source maps and workstation paths in module text, along with build claims for assets, extra modules, upload-metadata variables or secrets, tails, logging, observability, and routes. Its `workers_dev`, preview URL, route, export, ingress, and runtime-version fields are claimed local observations only. The candidate uses the direct beta API D1 binding field `database_id`; it does not claim byte identity with Wrangler 4.125.0 output. Local dry-run bundles still emit source maps, use synthetic build-time HTTP configuration, and lack deployed runtime-version evidence, so they do not pass. Unit tests simulate successful verified Access contexts. Workerd covers distinct role-only RPC modules and unauthenticated fetch denial, not deployed Access routing. The output exposes no raw upload bytes, is never upload-eligible or deployment-ready, and cannot advance lifecycle or gate state. No upload adapter exists. Version creation and Deployment still require separate lifecycle, reconciliation, readback, and cleanup work.

The lifecycle requires the opaque initialized-database result after `database_created`. It then records private shell creation, Version upload, and one-Version 100 percent Deployment for the sink, Writer A, and Writer B. Access follows in the fixed order application, service token, policy, then routes. The generic journal function cannot advance any of the nine Worker mutation steps, and the plain journal schema rejects every fabricated Worker-mutation prefix. A database-bootstrap failure before a Worker request may fail at `sink_private_shell_created` with the distinct `pre_dispatch` semantic. A dispatched shell create uses `ambiguous_create`; Version and Deployment use their specific ambiguity reasons. Every post-dispatch outcome is unknown, not retried, and requires manual cleanup.

The private operator package now contains a pure protocol compiler for the current beta Worker resource API and classic Scripts readbacks. It requires an empty shell, explicit `subdomain.enabled: false`, `previews_enabled: false`, disabled logpush and observability, no tail consumers, `deploy=false`, empty Version URLs, classic `hasPreview: false`, exact module and binding projections, a single Version at 100 percent, `force=false`, a second false/false subdomain readback, and matching runtime Version Metadata. It HMAC-commits Cloudflare identities with the shared role-free domains and binds shell creation to a bounded dispatch and observation window. Its output contains no raw Cloudflare IDs and sets mutation, upload, lifecycle, attestation, and gate authority to false.

`docs/fixtures/cloudflare-worker-api-canary.json` defines a narrower isolated interoperability canary with status `not_run`. It uses one fixed inert `entry.js` module with no bindings, D1 database, service binding, Access resource, route, `workers.dev`, preview URL, logging, observability, asset, or tail consumer. It may prove only beta empty-shell creation, beta Version creation with `deploy=false`, classic Version readback, one-Version classic Deployment with `force=false`, and beta deletion of the exact immutable Worker ID followed by exact-ID and complete-list absence. It does not invoke the Worker or test Access, so `beta_classic_api_canary_passed` and `access_protected_runtime_identity_canary_passed` remain separate and false. A result from this canary cannot advance the D1 lifecycle, satisfy runtime identity, become an attestation, or promote a gate.

The source-only Worker API canary runner accepts one canonical persisted operation plan, an HMAC key, and an account-scoped Workers Scripts Write token. One shared transport limits forward calls to GET and POST and cleanup calls to GET and DELETE. Both use the same redacted transcript and two-megabyte aggregate response budget. The runner rechecks the plan digest, derives and verifies the commitment-key identifier from the actual key bytes, enforces the five-minute operation window, follows no redirect, caps every response, and never retries a dispatched POST or DELETE. A checkout-local exclusive reservation blocks accidental concurrent or sequential reuse on one operator checkout; it is not a distributed authority fence. Every execution adds a fresh random ownership tag. Reconciliation and cleanup require that tag as well as the persisted ownership marker, so another host or worktree cannot adopt or delete a same-plan Worker. Cleanup freshly re-reads the immutable ID, generated name, both ownership tags, and private settings before its one beta immutable-ID delete. An unacknowledged delete remains `manual_required` even when later reads show absence. The retained result classifier distinguishes observed, inconclusive, manual-review, and interrupted outcomes, but no executable invokes it. No root or package command registers this runner, and its former executable module now returns only `worker_api_canary_disabled` before reading input or credentials. It remains unavailable until durable orchestration, cleanup resumption, response capture, and a secure secret-descriptor launcher land. It has not been run against Cloudflare in this repository.

`pnpm d1-probe:plan-worker-api-canary` is the non-mutating plan entry point. It accepts a canonical account request on standard input, reads only the HMAC key from FIFO or socket file descriptor 3, and generates the operation ID, suffix, markers, five-minute window, commitment-key identifier, and plan digest. It receives no API token and performs no network request. Separate unwired contracts now preserve the fresh execution tag and returned Worker, Version, and Deployment IDs in canonical `0600` immutable revision files under a `0700` checkout-local directory. Each revision is written and synchronized to a unique private temporary file, published once through an atomic hard link, and followed by a directory sync. The single-winner revision path makes `delete_dispatching` a durable one-way fence without a stale lock or empty final placeholder. An unwired cooperative driver lease uses the same append-only publication model, derives the production owner from the process, permits takeover only after expiry plus an `ESRCH` liveness result, and can reassert the exact current owner before a later effect. It is checkout-local coordination, not a distributed lock or security boundary, and the credentialed runner does not use it. A separate caller-constructible effect-claim journal stores only redacted operation, workflow, request, response, phase, timing, ambiguity, lease-epoch, and digest-chain claims. Its chain detects local corruption and reordering but does not authenticate a request, a Cloudflare response, or an external effect; it cannot close an operation or authorize mutation, cleanup, lifecycle, attestation, or gate promotion. Operation, lease, and effect records share one execution-nonce commitment, and every effect claim binds the exact lease generation and canonical lease-record digest that owned it. The public append API still accepts any structurally valid caller-built claim. A read-only consistency checker double-samples complete operation, effect, and lease histories, matches every claim to its exact stored operation revision and lease epoch, and rejects unresolved publication residue and unstable heads, then distinguishes missing, unfinished, ambiguous, synchronized, one-step-ahead, and corrupt local histories. Its output may become stale immediately. Any future effect must reassert the exact state, claim, and lease heads; the checker itself authorizes nothing. The shared transport has a one-use prepared-request path that calls a redacted ordering hook before `fetch`. A no-op adapter preserves the unregistered runner's current behavior. The hook does not prove that intent or start records reached durable storage, and the runner persists no claims through it. An unwired adapter can append exact intent and started claims while reasserting the current lease and state/journal heads. Forward claims require a null cleanup-obligation digest. Cleanup claims require the exact immutable cleanup-obligation record, bind its digest, use its separate grace window, and reassert the same record before fetch. The adapter is not atomic, authenticates no transport effect, and writes no terminal claim. A local failure after the started claim remains conservatively ambiguous. The transport also has an unwired caller-controlled response hook that receives one bounded byte copy after hashing and transcript recording. The callback may retain plaintext, and its completion proves neither archive publication nor claim authenticity. A separate response archive encrypts a bounded response body with AES-256-GCM after matching its digest and exact request context to a caller-constructible `response_observed` claim. The archive uses a per-claim HKDF key, records a redacted key identifier, and exports no plaintext. Its keyed internal resolver checks GCM, context, and the plaintext digest, then clears its plaintext and key copies. It grants no authority. Content type, identity encoding, and observation time remain caller assertions. An unwired joined response-claim adapter now binds one started claim to one response callback, builds the response claim, publishes the encrypted archive, reasserts the lease and exact heads, then appends that claim. Cleanup sessions also reassert the immutable obligation and preserve its digest through the response claim and archive context. Archive-before-append holds only when callers use this adapter. The direct public journal append remains caller-constructible. If archive publication succeeds and claim append fails, the archive is ahead. The read-only base classifier identifies that shape. An unwired keyed reducer now requires an active current lease, the exact unchanged started head, one adjacent archive record, and an unchanged inventory before it appends the reconstructed terminal claim. A takeover lease may authorize the local append, while the claim preserves the earlier lease epoch that owned the dispatch. It never repeats the remote request and returns no plaintext. A separate read-only archive inventory validates bounded canonical local envelope shapes and returns only redacted digests and safe scalars, including the cleanup-obligation digest for cleanup requests. It does not decrypt, prove key possession, authenticate an envelope or response, prove claim persistence, repair files, or authorize recovery. An unwired base classifier double-samples the operation, effect, lease, and archive-shape snapshots and emits only fixed redacted recovery requirements. It never retries a mutation, decrypts an archive, appends a claim, or transitions state. The credentialed runner uses neither adapter nor inventory, so end-to-end response-preimage capture remains unavailable. An unwired automatic-cleanup core accepts only a matching operation in `cleanup_reconciling`, runs during the independently derived cleanup grace, requires complete consistent pagination metadata, records `delete_dispatching` before one immutable-ID DELETE, and records `absence_observed` only after the acknowledged delete, exact-ID miss, and complete-list absence. A pure response-projection contract separates cleanup identity from stricter Version and Deployment semantics when Cloudflare omits optional response fields. `pnpm d1-probe:inspect-worker-api-canary-recovery` reads an existing local journal without credentials, network access, filesystem repair, or mutation. Its canonical output contains the plan digest, revision, operation state, retained-identity booleans, cleanup-window status, a fixed recovery status, and only false authority fields. It withholds names, resource IDs, ownership markers, state paths, and record bytes. None of these prerequisites is used by the credentialed runner yet. There is no secure secret-FD launcher, cleanup CLI, post-grace manual authority, integrated response-preimage capture, runner adoption of archive-ahead recovery, or checked-in observation transition. The checkout-local state is not a distributed lock and may not be treated as recoverable evidence after its private files are lost. A live run remains prohibited.

The local base layer now has a crash-aware driver bootstrap and a private durable-driver session for the pre-dispatch boundary. The bootstrap accepts only an exact revision-zero `prepared` operation and its exact cleanup grace, rejects any existing effect claim or response archive, creates or reuses the exact operation state and immutable cleanup obligation, acquires the cooperative lease, and reasserts all three records plus empty histories before returning. The durable session retains the lease owner and raw immutable execution identity in a closure. Its public surface exposes only the plan digest, execution-nonce commitment, cleanup-obligation digest, false authority, and a request-session factory. That factory revalidates the full immutable identity and composes the operation, private owner, workflow step, and caller-supplied archive key with the joined response-claim adapter. Exact concurrent publication winners may be reused; substitutions, hostile input objects, dependency failures, partial histories, and prior histories fail closed with stable denials. Cleanup-obligation reads compare only the exact plan and execution stem, so an unrelated plan publishing in the shared checkout does not create a false conflict. A failed final reassertion leaves the lease to expire or be conservatively taken over instead of releasing it into an uncertain race. The session prepares no request, reads no credential, sends no network call, prevents no caller retention of input material, and explicitly authorizes neither remote dispatch nor cleanup. The synthetic scalable base E2E uses this session and cannot directly import the lower-level bootstrap or joined response-claim factory. The credentialed runner does not use it. Restart after an intent or started claim must use recovery, never a fresh bootstrap.

The cleanup grace is now a pure shared contract rather than executor-local logic. An unwired immutable `0600` cleanup-obligation record binds that grace to the exact prepared operation and execution commitment before any remote effect. Its read-only verifier performs no repair or write. Publication completes only one exact pre-link residue and never overwrites a final record. Cleanup dispatch and response claims require its digest; encrypted archive AAD, redacted inventory, consistency, base recovery, and archive-ahead reconciliation preserve and compare the same digest. This is structural continuity across caller-constructible local records, not authentication or cleanup authority. The credentialed runner does not write or consume the obligation.

The production artifact, protocol, and lifecycle now share the versioned beta JSON Version contract and its bare artifact, binding-configuration, and Version-request digests. Cross-role vectors prove matching canonical projections and digests for the sink, Writer A, and Writer B. This resolves only the local identity mismatch. Production upload remains blocked because the isolated beta-to-classic canary is still `not_run`, first-Version preview safety is unresolved, and no credentialed private-shell adapter or opaque lifecycle evidence consumer exists. The isolated canary and a later production adapter remain separate.

The deployed D1 Writer trigger must use the implemented HTTP handler already wired into each local role-only entrypoint. It binds one canonical HTTPS route, an exact Worker Access audience and service-client identity, the writer role, canonical JSON, and a 16,384-byte body limit. It must receive Cloudflare's verified Access context on the direct request and does not parse Access headers. Every authenticated response reports canonical runtime version metadata for later comparison with deployment evidence; pre-Access denials omit it.

`packages/d1-probe-driver` owns the operator-side Writer transport. It sends one canonical request with the standard Access Client ID and Client Secret headers, follows no redirects, retries nothing, keeps the timeout active through the last response byte, and caps the response at 65,536 bytes. It requires the exact HTTP status, request digest, writer role, canonical JSON, no-store response headers, and one canonical runtime-version header on every Access-authenticated response. It returns the exact version ID, tag, and timestamp for later collector binding. Missing, malformed, noncanonical, or pre-Access-leaked metadata denies the response. Network loss, timeout, an oversized body, or any mismatch stays `outcome_unknown`. The package is private and cannot enter product, authority, or Worker code.
