...
Before any remote effect, a crash-aware bootstrap creates or reuses the exact prepared operation state and immutable private cleanup-obligation record, rejects prior effect or archive history, acquires the cooperative lease, and reasserts all local heads. The obligation binds the prepared operation, execution commitment, and independently digested cleanup grace. Its read-only verifier changes no file and scopes concurrent snapshots to the exact plan and execution stem. Cleanup dispatch, response, encrypted archive, inventory, consistency, and recovery contracts carry and reassert that exact obligation digest; forward claims must keep it null. The bootstrap, record, and claims remain caller-constructible, the credentialed runner does not consume them, and none authenticates a remote effect or authorizes dispatch or cleanup.
...
|---|---|---|
| Durable identity and state | The bootstrap establishes exact prepared state, cleanup obligation, empty histories, and current lease; later operation, effect, and archive records share the same plan and execution bindings | Make the credentialed driver call the bootstrap before it creates the composed dispatch-response session |
...
The local base layer now has a crash-aware driver bootstrap for the pre-dispatch boundary. It accepts only an exact revision-zero `prepared` operation and its exact cleanup grace, rejects any existing effect claim or response archive, creates or reuses the exact operation state and immutable cleanup obligation, acquires the cooperative lease, and reasserts all three records plus empty histories before returning. Exact concurrent publication winners may be reused; substitutions and partial or prior histories fail closed. Cleanup-obligation reads compare only the exact plan and execution stem, so an unrelated plan publishing in the shared checkout does not create a false conflict. A failed final reassertion leaves the lease to expire or be conservatively taken over instead of releasing it into an uncertain race. The returned session contains private local material for a future driver but explicitly authorizes neither remote dispatch nor cleanup. It does not authenticate the caller-supplied operation, execution tag, or cleanup obligation. The synthetic scalable base E2E uses this bootstrap; the credentialed runner does not. Restart after an intent or started claim must use recovery, never this fresh bootstrap.
