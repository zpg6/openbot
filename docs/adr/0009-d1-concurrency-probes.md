...
The checked-in Wrangler files are local-only build inputs. They disable `workers.dev`, preview URLs, logs, and routes. Both writers bind one local D1 database and the exact `D1ProbeSinkService` entrypoint. `pnpm d1-probe:build-local` performs dry-run bundles only and has no deploy mode. The later deployment driver must generate new configs from the preflight plan and add the exact Access-protected trigger and readback routes, collector, and cleanup. The current private Workers neither accept public probe traffic nor close a gate.

The unwired Writer HTTP adapter defines the future trigger boundary without creating a route. Bootstrap configuration pins one canonical HTTPS URL, one Access audience, one Access service-token client ID, and one writer role. The adapter accepts only an exact `POST` with unencoded `application/json`, an exact content length, and at most 16,384 canonical UTF-8 bytes. It rejects query strings, alternate paths, duplicate JSON keys, noncanonical JSON, role substitution, and malformed execution results. It reads only Cloudflare's verified `ctx.access` audience and service-token identity. It does not parse an unverified Access header. The deployed entrypoint must pass the context from the directly authenticated request because Access context does not cross the private sink binding.

The private Node-only driver transport sends the matching request. It uses Cloudflare's standard `CF-Access-Client-Id` and `CF-Access-Client-Secret` headers, sends one canonical body, follows no redirects, and makes no automatic retry. Its timeout remains active while it reads the response body. The response cap is 65,536 bytes. A response is usable only when its status, request digest, writer role, canonical JSON bytes, and no-store headers match the Writer contract. A network error, timeout, redirect, oversized body, invalid UTF-8, or substituted response becomes `outcome_unknown`. The transport returns no credential field or exception text.
