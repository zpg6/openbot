...
The candidate may become `enabled` only after an operator-controlled authoritative-domain probe observes no randomized sentinel from every enumerated Node DNS path and image binary during the fixed observation window. A pass is evidence for those observed paths on the exact image, runtime, SDK, configuration digest, and recorded platform fingerprint. The dated evidence has a `valid_until`; expired evidence denies. It is not a universal no-egress claim. The enabled profile may accept public, synthetic, or organization data before organization policy and grant narrowing; `restricted` and `unknown` remain denied. Any failed or incomplete probe blocks the code-enabled release and requires another executor or containment design.
