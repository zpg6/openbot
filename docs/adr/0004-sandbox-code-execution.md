...
- `enabled` requires a complete adoption-evidence record for the immutable `configuration_digest`. The record names its own digest, `observed_at`, `valid_until`, `cloudflare_platform_fingerprint`, and every typed check as passed. Evidence that is expired or belongs to another platform or configuration denies. An enabled profile may accept inherited `public`, `synthetic`, or `organization` data. It always denies `restricted` or `unknown` data with `sandbox_data_profile_denied`.
...
- Treat a passing sentinel as observed coverage of the reviewed image and enumerated paths, not proof that DNS exfiltration is impossible. Record image digest, Node version, SDK version, probe source digest, authoritative zone, attempted mechanisms, observation interval, log digest, `cloudflare_platform_fingerprint`, `observed_at`, `valid_until`, and result. Expired evidence denies. Re-run after any image, runtime, SDK, network, or platform-behavior change.
