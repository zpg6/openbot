...
After all release-blocking probes pass, the reviewed profile may receive `enabled` adoption status. It must also remain `active` and carry complete unexpired adoption evidence for its immutable `configuration_digest` and recorded `cloudflare_platform_fingerprint`. The evidence has its own digest, `observed_at`, `valid_until`, and typed passed checks. The record's `profile_digest` changes with adoption status, lifecycle, or evidence; the `configuration_digest` does not. A candidate, disabled, expired-evidence, or fingerprint-mismatched profile returns `sandbox_profile_not_enabled` for every user run.
