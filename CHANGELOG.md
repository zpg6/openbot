...
- Added an unwired read-only consistency checker for the canary operation history and effect-claim journal. It rejects unresolved publication residue, historical binding drift, unstable heads, impossible ordering, and multi-step gaps. Its redacted snapshot may become stale and cannot authorize work.
- Added a one-use caller-controlled pre-dispatch ordering hook and a write-only encrypted response-body archive for the Worker canary. The current runner persists no claims through the hook and writes no archive records. Neither addition authenticates Cloudflare effects or grants mutation authority.
