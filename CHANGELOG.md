...
- A private D1 probe network transport that sends one canonical Access service-token request, refuses redirects and retries, binds the Writer response to the request and role, and treats timeouts or malformed responses as unknown. It has no CLI, stored credential, route, or deployment command.
