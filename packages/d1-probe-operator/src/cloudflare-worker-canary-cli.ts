...
const recordTerminationRequest = (): void => {
    terminationRequested = true;
};
process.once("SIGINT", recordTerminationRequest);
process.once("SIGTERM", recordTerminationRequest);
...
const main = async (): Promise<void> => {
    ...
    try {
        const result = await executeD1ProbeCloudflareWorkerCanaryCommandV1(command, hmacKey, apiToken, {
            fetch: globalThis.fetch,
            now: Date.now,
            shouldTerminate: () => terminationRequested,
        });
        if (!result.success) return await fail(result.code);
        await writeLine(process.stdout, canonicalizeJsonV1(result.result as unknown as CanonicalJsonValueV1));
        const outcome = classifyD1ProbeCloudflareWorkerCanaryProcessOutcomeV1(result.result, terminationRequested);
        if (outcome.stderr_code !== null) await writeLine(process.stderr, outcome.stderr_code);
        process.exitCode = outcome.exit_code;
    } catch {
        await fail("worker_api_canary_internal_error");
    }
};

await main();
process.removeListener("SIGINT", recordTerminationRequest);
process.removeListener("SIGTERM", recordTerminationRequest);
