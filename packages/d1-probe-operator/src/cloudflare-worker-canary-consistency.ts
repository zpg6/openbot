...
export interface D1ProbeCloudflareWorkerCanaryConsistencyV1 {
    ...
    readonly missing_component: "state" | "effect_journal" | "both" | null;
    readonly corrupt_component: "input" | "state" | "effect_journal" | "bindings" | null;
...
> => {
    const [firstState, firstJournal] = await Promise.all([
        readers.read_state_history(planDigest),
        readers.read_effect_journal(planDigest),
    ]);
    const [firstStateSignature, firstJournalSignature] = await Promise.all([
        stateSignature(firstState),
        Promise.resolve(journalSignature(firstJournal)),
    ]);
    const [secondState, secondJournal] = await Promise.all([
        readers.read_state_history(planDigest),
        readers.read_effect_journal(planDigest),
    ]);
    const [secondStateSignature, secondJournalSignature] = await Promise.all([
        stateSignature(secondState),
        Promise.resolve(journalSignature(secondJournal)),
    ]);
    return firstStateSignature === secondStateSignature && firstJournalSignature === secondJournalSignature
        ? { stable: true, state: secondState, journal: secondJournal }
        : { stable: false };
};
...
const classifyStableSnapshot = async (
    ...
    if ((!stateResult.success && !stateMissing) || (!journalResult.success && !journalMissing)) {
        return report(planDigest, "corrupt", null, !stateResult.success && !stateMissing ? "state" : "effect_journal");
    }
    ...
    if (stateMissing && journalMissing) return report(planDigest, "missing", "both", null);
    ...
    if (claimHead.effect_phase === "dispatch_started" || claimHead.effect_phase === "dispatch_ambiguous") {
        return report(planDigest, "ambiguous_dispatch", null, null, stateHead, latest);
    }
    if (stateOneAhead) {
        return report(planDigest, "state_ahead", null, null, stateHead, latest);
    }
    if (claimHead.effect_phase === "dispatch_intent") {
        return report(planDigest, "claim_behind", null, null, stateHead, latest);
    }
    return report(planDigest, "exact_sync", null, null, stateHead, latest);
};
...
export const readD1ProbeCloudflareWorkerCanaryConsistencyWithReadersTestOnlyV1 = async (
    ...
    try {
        ...
        return snapshot.stable
            ? await classifyStableSnapshot(planDigestInput, snapshot.state, snapshot.journal)
